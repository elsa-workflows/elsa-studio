...
- `Elsa.Studio.Authentication.ElsaAuth.BlazorServer` - For Blazor Server applications
- `Elsa.Studio.Authentication.ElsaAuth.BlazorWasm` - For Blazor WebAssembly applications
...
- **`ElsaAuthHttpConnectionOptionsConfigurator`** - Configures SignalR connections with JWT authentication

### HTTP Message Handlers

- **`ElsaAuthAuthenticatingApiHttpMessageHandler`** - Automatically adds JWT bearer tokens to HTTP requests to the Elsa backend API
...
Both extensions call `AddElsaAuthCore()` from this module to register the shared services, then add their platform-specific `IJwtAccessor` implementation.
...
5. `ElsaAuthAuthenticatingApiHttpMessageHandler` automatically adds tokens to API requests
...
- **`Elsa.Studio.Authentication.ElsaAuth.BlazorServer`** - Blazor Server hosting implementation
- **`Elsa.Studio.Authentication.ElsaAuth.BlazorWasm`** - Blazor WebAssembly hosting implementation
- **`Elsa.Studio.Authentication.ElsaAuth.UI`** - Login UI components and pages
...
1. API requests detect expired tokens via `ElsaAuthAuthenticatingApiHttpMessageHandler`
