...
public sealed class ConnectionEditorTests : BunitContext, IAsyncLifetime
{
    ...
    public void GenericEditor_BlocksSavingAfterInvalidJsonInput()
    {
        ...
        var adapter = new AdapterDescriptor { Type = "custom", DisplayName = "Custom", Fields = [new ConnectionFieldDescriptor { Name = "projection", DisplayName = "Claim projection", ValueType = "string-array" }] };
        var cut = Render<ConnectionEditor>(parameters => parameters
            .Add(component => component.Connection, CreateConnection())
        ...
        Assert.False(saved);
        Assert.Contains("valid JSON array", cut.Markup, StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        var menu = new ExternalAuthenticationMenu(new FeatureProvider(true), new PermissionService(true));

        var visible = await menu.GetMenuItemsAsync();
        var item = Assert.Single(visible, candidate => candidate.Href == "security/external-authentication");
        Assert.Equal("security/external-authentication", item.Href);

        var hidden = await new ExternalAuthenticationMenu(new FeatureProvider(true), new PermissionService(false)).GetMenuItemsAsync();
        Assert.Empty(hidden);
    }
    ...
    private static AdapterDescriptor CreateAdapter(bool includeSecret = false, bool includeUnsafe = false)
    {
        ...
        if (includeSecret)
            descriptor.Fields.Add(new ConnectionFieldDescriptor { Name = "clientSecret", DisplayName = "Client secret", IsSecretBinding = true });
        if (includeUnsafe)
    ...
    private sealed class TestConnectionsApi : IExternalAuthenticationConnectionsApi
    {
