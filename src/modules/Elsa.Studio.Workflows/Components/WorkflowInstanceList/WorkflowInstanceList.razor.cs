...
public partial class WorkflowInstanceList : IAsyncDisposable
{
    ...
    private ICollection<WorkflowSubStatus> SelectedSubStatuses { get; set; } =
        new List<WorkflowSubStatus>();

    // The selected timestamp filters to filter by.
    private ICollection<TimestampFilterModel> TimestampFilters { get; set; } =
        new List<TimestampFilterModel>();
    ...
    private async Task<TableData<WorkflowInstanceRow>> LoadData(
        TableState state,
    ...
    private async Task<PagedListResponse<WorkflowInstanceSummary>> TryListWorkflowDefinitionsAsync(
    ...
    private void OnRowClick(TableRowClickEventArgs<WorkflowInstanceRow> e) =>
    ...
    {
        var reference = await DialogService.ShowAsync<BulkCancelDialog>(
    ...
    private async Task OnSelectedWorkflowDefinitionsChanged(
        IEnumerable<WorkflowDefinitionSummary> values
    )
    {
